...
# matching trait
trait.pruned <- shape.v.2d.means[which(rownames(shape.v.2d.means) %in% tree.pruned$tip.label),]
# community
...
save (RAO_OBS, file = here("output_concensus_supp_S5","RAO_OBS.RData"))
...
# estimating parameters to be used in trait simulation
# contrained BM
simul_params_BM<-mvgls(scale(trait.pruned) ~ 1,  
                       tree=tree.pruned, 
                      model = "BM", 
                      method=("PL-LOOCV"),
                      REML=T)

save (simul_params_BM,
      file=here ("output_concensus_supp_S5",
                 "params_BM.RData"))

# simulating traits using a Brownian motion model
# 100 simulations per phylogeny
simul_BM<- mvSIM(tree.pruned,
                 nsim=niter,
                 model="BM1",
                 param = list(sigma = (simul_params_BM$sigma$S),
                              theta = rep(0,nrow(simul_params_BM$sigma$S)),
                              ntraits = nrow(simul_params_BM$sigma$S)))

# Get average values
simul_BM_mean <- Reduce("+",simul_BM)/length(simul_BM)

# Run the disparity function that organizes data and calculate observed and null Rao's entropy
# run
RAO_BM <-   funcao_disparidade (occ = data.matrix(comm.pruned),
                      traits= as.data.frame(simul_BM_mean),
                      n_iterations = niter)


# save
save(RAO_BM, file=here("output_concensus_supp_S5","RAO_BM.Rdata"))

# simulating traits using a early burst model
# 100 simulations per phylogeny
simul_params_EB <-mvgls(scale(trait.pruned) ~ 1,  
                                 tree=tree.pruned, 
                                 model = "EB", method=("PL-LOOCV"),
                                 REML=T)
# simulate traits
simul_EB <- mvSIM(tree.pruned,
                 nsim=niter,
                 model="EB",
                 param = list(sigma = (simul_params_EB$sigma$S),
                              theta = rep(0,nrow(simul_params_EB$sigma$S)),
                              beta = simul_params_EB$coefficients,
                              ntraits = nrow(simul_params_EB$sigma$S)))
...
stopCluster (cl)
# save
save (RAO_EB, file = here("output_concensus_supp_S5","RAO_EB.RData"))

# simulating traits using a Ornstein-Uhlenbeck model
# 100 simulations per phylogeny

# 100 simulations per phylogeny
simul_params_OU <-mvgls(scale(trait.pruned) ~ 1,  
                        tree=tree.pruned, 
                        model = "OU", method=("PL-LOOCV"),
                        REML=T)
# simmap tree
teste <- make.simmap(tree.pruned, scale(trait.pruned), 
                     model="SYM", nsim=10)

# simulate traits
simul_OU <- mvSIM(tree.pruned,
                  nsim=niter,
                  model="OU",
                  param = list(sigma = (simul_params_EB$sigma$S),
                                          theta = rep(0,nrow(simul_params_EB$sigma$S)),
...
save (RAO_OU, file = here("output_concensus_supp_S5","RAO_OU.RData"))
...
# save
save (null.mpdf, file=here ("output_concensus_supp_S5","mpd_results.RData"))