...
# -----------------------#
# load data
load (here("output_sensitivity_supp_S4", "mpd_results_ALL.RData"))
load (here("output_sensitivity_supp_S4", "RAO_BM_ALL.RData"))
load (here("output_sensitivity_supp_S4", "RAO_EB_ALL.RData"))
load (here("output_sensitivity_supp_S4", "RAO_OU_ALL.RData"))
load (here("output_sensitivity_supp_S4", "RAO_OBS_ALL.RData"))
load (here("output_sensitivity_supp_S4", "parameters.RData"))
...
test_betas <- lapply (seq (1,length(sigmas_sub)), function (i) 
  
  
  rbind (cbind(RAO_OBS$SES,"EMPIRICAL", "Empirical"),
                      cbind (RAO_EB_sub[[i]][[1]]$SES,paste("beta",betas_sub[1],sep=""),
                             paste("sigma", sigmas_sub[i], sep="")),
                      cbind (RAO_EB_sub[[i]][[2]]$SES,paste("beta",betas_sub[2],sep=""),
                             paste("sigma", sigmas_sub[i], sep="")),
                      cbind (RAO_EB_sub[[i]][[3]]$SES,paste("beta",betas_sub[3],sep=""),
                             paste("sigma", sigmas_sub[i], sep="")),
                      cbind (RAO_EB_sub[[i]][[4]]$SES,paste("beta",betas_sub[4],sep=""),
                             paste("sigma", sigmas_sub[i], sep="")))
)

# melt
test_betas<-do.call(rbind, test_betas)

#bind MPD
test_betas<-cbind(test_betas, null.mpdf$SES.MPD)
test_betas <- data.frame(test_betas)# turn into df
colnames(test_betas)<-c("SES.Disparity","Dataset","Sigma","SES.MPD")
test_betas$SES.Disparity <-as.numeric(test_betas$SES.Disparity)# change class
test_betas$Dataset <-as.factor(test_betas$Dataset)
test_betas$Sigma <-as.factor(test_betas$Sigma)
...
test_alphas <- lapply (seq (1,length(sigmas_sub)), function (i) 
  
  
  rbind (cbind(RAO_OBS$SES,"EMPIRICAL", "Empirical"),
         cbind (RAO_OU_sub[[i]][[1]]$SES,paste("alpha",alphas_sub[1],sep=""),
                paste("sigma", sigmas_sub[i], sep="")),
         cbind (RAO_OU_sub[[i]][[2]]$SES,paste("alpha",alphas_sub[2],sep=""),
                paste("sigma", sigmas_sub[i], sep="")),
         cbind (RAO_OU_sub[[i]][[3]]$SES,paste("alpha",alphas_sub[3],sep=""),
                paste("sigma", sigmas_sub[i], sep="")),
         cbind (RAO_OU_sub[[i]][[4]]$SES,paste("alpha",alphas_sub[4],sep=""),
                paste("sigma", sigmas_sub[i], sep="")))
)

# melt
test_alphas<-do.call(rbind, test_alphas)

#bind MPD
test_alphas<-cbind(test_alphas, null.mpdf$SES.MPD)
test_alphas <- data.frame(test_alphas)# turn into df
colnames(test_alphas)<-c("SES.Disparity","Dataset","Sigma","SES.MPD")
...
test_alphas$Sigma <-as.factor(test_alphas$Sigma)
